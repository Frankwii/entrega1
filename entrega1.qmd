...
!\[AAAAAA\]{Explicacion_dimensiones_pico.png}
...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species)) +
  ...
  labs(x = "Longitud del Pico (mm)", y = "Profundidad del pico (mm)",
       title = "Profundidad del pico según su longitud, por especie") +
...
ggplot(penguins, aes(x = body_mass_g, y = flipper_length_mm, color = species)) +
  ...
  labs(x = "Masa corporal (g)", y = "Longitud de la aleta (mm)",
       title = "Longitud de la aleta según masa corporal, por especie") +
  scale_color_manual(values=species_colors)
```
...
ggplot(penguins, aes(x = bill_length_mm,
                     y = flipper_length_mm,
                     color = species,
                     shape = sex %>% as.character %>% replace(is.na(.), "NA") %>% as.factor)) +
  geom_point() + 
  labs(title = "Longitud del pico según la de la aleta, por especie y sexo",
...
::: {layout-ncol="2"}
```{r, echo=FALSE}
ggplot(penguins, aes(x = sex, y = bill_length_mm, fill = sex)) +
  geom_violin() +
  geom_boxplot(width=0.2, fill="lightgray") +
  labs(x = "Especie de Pingüino", y = "Longitud del Pico (mm)",
       title = "Longitud del pico por Sexo") +
  scale_fill_brewer(palette = "Dark2") +
  guides(fill=guide_legend(title="Especie"))
```

```{r, echo=FALSE}
ggplot(penguins, aes(x = sex, y = bill_depth_mm, fill = sex)) +
  geom_violin() +
  geom_boxplot(width=0.2, fill="lightgray") +
  labs(x = "Especie de Pingüino", y = "Profundidad del Pico (mm)",
       title = "Profundidad del pico por Sexo") +
  scale_fill_brewer(palette = "Dark2")+
  guides(fill=guide_legend(title="Especie"))
```
...
ggplot(penguins, aes(x = sex, y = flipper_length_mm, fill = sex)) +
  ...
  geom_boxplot(width=0.2, fill="lightgray") +
  labs(x = "Especie de Pingüino", y = "Longitud de la aleta (mm)",
       title = "Longitud de la aleta por Sexo") +
  scale_fill_brewer(palette = "Dark2")+
  guides(fill=guide_legend(title="Especie"))
```
...
ggplot(penguins, aes(x = sex, y = body_mass_g, fill = sex)) +
  ...
  geom_boxplot(width=0.2, fill="lightgray") +
  labs(x = "Especie de Pingüino", y = "Masa corporal (g)",
       title = "Masa corporal por Sexo") +
  scale_fill_brewer(palette = "Dark2")+
  guides(fill=guide_legend(title="Especie"))
```
:::

Según el gráfico, parece que los pingüinos presentan dimorfismo sexual: los machos suelen ser más grandes que las hembras respecto a todas las medidas. A simple vista, no se reconoce ningún patrón que sigan los pingüinos sin sexo registrado.

Podría ser interesante separar este estudio por especie y estudiar completamente cada una, pero por brevedad nos hemos limitado a realizar un estudio global (además, ya hemos realizado un estudio muy relacionado en el apartado anterior). 
...
Esta distribución tan heterogénea de los pingüinos por isla nos indica que, muy posiblemente, no sea interesante estudiar cómo se relaciona esta variable con las demás, pues está muy sesgada por la especie y ya hemos estudiado con detalle esta última.
