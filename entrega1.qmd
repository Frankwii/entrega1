...
c(x,"Total no NA"=sum(x,na.rm=TRUE))
```
...
c(x,"Total_noNA"=sum(x,na.rm=TRUE))
```
...
"El pico más corto mide r{estadisticos[1]} milímetros, y el más largo `r estadisticos[6]` milímetros. El primer cuartil de esta variable es `r estadisticos[2]`, la mediana `r tadisticos[3]` y el tercero `r estadisticos[5]`. La media es de `r estadisticos[4]` y la desviación típica muestral de `r sd(penguins$bill_length_mm)`. Además, hay `r estadisticos[7]` valores NA."
